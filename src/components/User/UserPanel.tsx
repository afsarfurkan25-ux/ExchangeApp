...
const UserPanel: React.FC = () => {
    ...
    const loadUserSettings = (): UserPanelSettings => {
        ...
        let savedQrUrl = settings.qrUrl;
        if (saved) {
            try {
                const parsed = JSON.parse(saved);
                if (parsed.scrollingText) savedScrollingText = parsed.scrollingText;
                if (parsed.qrUrl) savedQrUrl = parsed.qrUrl;
            } catch { /* fall through */ }
        }
    ...
    const [localQrUrl, setLocalQrUrl] = useState(userSettings.qrUrl || "");
                        ...
                        <div style={{
                            ...
                            <div style={{ marginBottom: '24px' }}>
                                <label style={{
                                    ...
                                    marginBottom: '8px', fontWeight: 600, textTransform: 'uppercase',
                                    letterSpacing: '1px',
                                }}>QR KOD LİNKİ (Bizi Takip Edin)</label>
                                <input
                                    type="text"
                                    value={localQrUrl}
                                    onChange={(e) => setLocalQrUrl(e.target.value)}
                                    placeholder="Örn: https://instagram.com/dukkanadi"
                                    style={{
                                        width: '100%',
                                        padding: '14px 18px',
                                        background: 'rgba(255, 255, 255, 0.04)',
