...
const Clock: React.FC = () => {
    const { settings, currentUser } = useExchange();
    ...
    // Determine QR URL
    let qrUrlToDisplay = settings?.qrUrl || "http://alkanmedya.com";
    if (currentUser?.name) {
        const userSettingsKey = `userPanelSettings_${currentUser.name}`;
        const savedSettings = localStorage.getItem(userSettingsKey);
        if (savedSettings) {
            try {
                const parsed = JSON.parse(savedSettings);
                if (parsed.qrUrl) qrUrlToDisplay = parsed.qrUrl;
            } catch { /* ignore */ }
        }
    }
